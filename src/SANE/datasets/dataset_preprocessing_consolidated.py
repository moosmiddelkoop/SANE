"""Consolidated variant of dataset_preprocessing.py: instead of writing one .pt
file per sample into dataset_torch.{split}/ dirs, stacks each split into in-RAM
tensors and saves everything as one <target>/dataset.pt (a {"trainset","valset",
"testset"} dict of TensorSamplingDataset) that training reads sequentially in
one go. Use dataset_preprocessing.py for the original per-sample-file output."""
...
def prepare_multiple_datasets(configurations: List[Dict[str, Any]]):
    """
    Prepares datasets for multiple architectures. Collects the stacked splits per
    target path and saves them as one consolidated <target>/dataset.pt (a
    {"trainset","valset","testset"} dict of TensorSamplingDataset), so training
    loads the whole dataset with one sequential read.
    Args:
    ...
    for config in configurations:
        ...
        collected.setdefault(Path(config["dataset_target_path"]), {}).update(
...
def stack_dataset(dataset, num_workers=0):
    ...
    logging.info(f"Number of samples to be stacked: {n}")
    for i, (ddx, mask, pos, prps) in tqdm(enumerate(dataloader), total=n):
        # remove batch dimension
        w[i], m[i], p[i], props[i] = ddx.squeeze(0), mask.squeeze(0), pos.squeeze(0), prps.squeeze(0)

    return TensorSamplingDataset(w, m, p, props)
...
def prepare_dataset(
    ...
    Returns:
        dict mapping each split to its stacked TensorSamplingDataset.
    """
    ...
    for split_key in splits:
        ...
        datasets[split_key] = preprocess_single_split(
...
def preprocess_single_split(
    ...
    Returns:
        the split stacked into a TensorSamplingDataset.
    """
